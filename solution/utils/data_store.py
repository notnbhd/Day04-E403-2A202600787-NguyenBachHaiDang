...
def _normalize(text: str) -> str:
    """Normalize text for search: remove diacritics, lowercase, strip punctuation."""
...
class OrderDataStore:
    """
    Manages the product catalog, discount simulation, order totals calculation,
    and order persistence for the order-agent lab.
    """
    ...
    def build_detail_token(product_ids: list[str]) -> str:
        """Build a deterministic validation token from sorted product IDs."""
    ...
    def list_products(
        ...
        *,
        ...
        """
        Search the product catalog using text matching, category filters,
        price ceiling, and tag requirements. Returns compact summaries
        suitable for the agent to pass to get_product_details.
        """
    ...
    def get_product_details(self, product_ids: list[str]) -> dict:
        """
        Return exact pricing, stock, category, and warranty for each product ID.
        Also returns a deterministic detail_token for later validation.
        """
        details: list[dict] = []
    ...
    def get_discount(self, *, seed_hint: str, customer_tier: str = "standard") -> dict:
        """
        Simulate a campaign discount with deterministic seeding.
        Returns 0.1 or 0.2 discount rate based on seed hash.
        """
    ...
    def calculate_order_totals(self, *, items: list[OrderLineInput], detail_token: str, discount_rate: float) -> dict:
        """
        Validate product IDs, detail token, and stock. Compute subtotal,
        discount amount, and final total. Returns error payload for failures.
        """
    ...
    def save_order(
        ...
        *,
        ...
        """
        Recompute totals, build a deterministic order ID, persist the order
        to a JSON file, and return the saved payload with file path.
        """
